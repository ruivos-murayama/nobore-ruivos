...
> バスケでリングに当たらずネットだけを鳴らす、あの一発。北極星その1（弾けて跳ねる連鎖）と
> 対になる別系統の気持ちよさ＝**読みでなく"狙って当てる"精度**。雑な弧はリムに弾かれて台無し、
> ど真ん中はスウィッシュで回復＋大演出＋連鎖。**第9章 月輪・9-1「無音」がこの北極星ステージ。**
...
- **フープ（輪）＝スウィッシュの的**（`gen.hoopCount`／`gimmick.hoopGap`/`hoopRimR`/`hoopRimRestitution`/`swishRefund`/`swishChainPitch`）：中央の開けた空間に**上向きの輪**を段違いに吊るす。各輪は2本のリム支柱＋中央の開口。**弾道セグメント(前位置→現位置)が開口面を順方向に横切り、横ズレ<gap、かつリムに触れていなければ「スウィッシュ！」**＝飛ばし回数回復＋大演出＋`swishChain`連鎖。リム支柱に触れると**弾かれて(`hoopRimRestitution`<1で減衰＝無限バウンドしない)「カチッ」**・連鎖リセット（舐めて通れば「イン」＝回復なし）。バンパー（読みの反射）/カタパルト（設計された大跳躍）と対になる「**狙って当てる精度**」の快感＝🌟北極星その2。**輪は中央・左右壁は素通しで登れる＝到達には必須でない**（ソルバは輪を無視＝幾何到達性のみ検証／詰み防止）。判定はテレポート系(バンパー/カタパルト)より前＝純弾道で評価。9-1「無音」の主役。描画 `drawHoops`（発光リム＋白ネット＋射抜く向きのシェブロン＋誘導ハロー、射抜くと点滅して靡く）。
