...
- **Gimmicks**: bumpers (`bouncy`), updrafts (`boosts`), **catapults** (`catapults`, fixed-vector launch pads — ignore incoming velocity and fire you along a set direction at `gimmick.catapultPower`; the "designed burst" counterpart to skill-based bumpers, chainable via the shared `bumpChain`), **hoops** (`hoops`, basketball-style rings — two small bouncy rim posts with an opening; threading the opening *forward* without grazing a post = スウィッシュ → refunds `gimmick.swishRefund` launches + `swishChain` combo; grazing a rim post deflects you with `gimmick.hoopRimRestitution`<1 (decays, never infinite-bounces) and breaks the chain. Detection uses the ballistic segment `blob.px,py → x,y` and runs *before* teleport gimmicks so it reads pure flight), moving platforms (`platforms`), slip walls (`slipWalls`, slowly slide down), guards (`sentries`, sweeping vision cones blocked by walls), cloak drops (`cloaks`, temporary invisibility). Counts are set per stage in `gen` (`bouncyCount`, `boostCount`, `catapultCount`, `hoopCount`, `platCount`, `slipCount`, `sentryCount`, `cloakCount`, `hazardCount`). Catapults and hoops are pure juice — never required for reachability (the BFS solver ignores them), so stages stay solvable via wall-climb alone.
